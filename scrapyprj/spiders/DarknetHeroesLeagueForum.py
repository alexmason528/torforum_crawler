...
class DreamMarketForumSpider(ForumSpider):
    ...
    def get_user_item_from_postwrapper(self, postwrapper, response):
        ...
        m = re.search('(\d+)', self.get_text(extrainfo.css("li.karma")))
        if m:
            useritem['reputation_power'] = m.group(1)

        useritem['rep_bars'] = str(len(extrainfo.css("li.repbars img")))
        useritem['stars'] = str(len(extrainfo.css("li.stars img")))

        return useritem

    def parse_userprofile(self, response):
        raise Exception("User profile not implemented as I have no user allowed to access them at this day.")

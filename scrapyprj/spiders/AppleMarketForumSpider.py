...
class AppleMarketForumSpider(ForumSpider):
    name = "applemarket_forum"
    ...
    def craft_login_request_from_form(self, response):
        ...
        captcha_src = response.css("#captcha_securimage_image::attr(src)").extract_first()

        req.meta['captcha'] = {        # CaptchaMiddleware will take care of that.
            'request' : self.make_request('captcha_img', url=captcha_src),
            'name' : 'captcha_securimage_code',    # Preprocess image to extract what's within the rectangle
            }
